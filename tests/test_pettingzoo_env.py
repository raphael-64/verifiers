...
class MockClient:
    """Minimal mock satisfying client.chat.completions.create(...)."""

    def __init__(self, content: str = "0"):
    ...
    async def create(self, *, model: str, messages, **kwargs):
        message = ChatCompletionMessage.model_construct(
            role="assistant", content=self._content, tool_calls=None
        )
...
def test_env_kwargs_passed_to_env():
    ...
    res = env.evaluate_sync(
        client=MockClient(), model="mock", state_columns=["pz_done"]
    )
    st = res["outputs"][0]
    assert st.get("pz_done") is True
...
def test_action_mask_in_prompt():
    ...
    res = env.evaluate_sync(client=MockClient(), model="mock")
    st = res["outputs"][0]
    first_prompt = st["prompt"]
...
def test_invalid_action_ends_episode():
    ...
    res = env.evaluate_sync(
        client=MockClient(content="999999"),
        model="mock",
